Процедура СохранитьВременноеЗначениеОборудования(ИмяПараметра, ЗначениеПараметра) Экспорт 
	ХранилищеОбщихНастроек.Сохранить(ИмяНастроек(), ИмяПараметра, ЗначениеПараметра);	
КонецПроцедуры
 
Функция ПолучитьВременноеЗначениеОборудования(ИмяПараметра) Экспорт
	Возврат ХранилищеОбщихНастроек.Загрузить(ИмяНастроек(), ИмяПараметра);
КонецФункции
...
Функция СтандартПоддерживается() Экспорт	
	Возврат ЭмуляторKSOFT_ОбщегоНазначенияПовтИсп.СтандартПоддерживается();
КонецФункции

Функция ИдентификаторКлиента(СсылкаНаОборудование) Экспорт
	Возврат СтрЗаменить(XMLСтрока(СсылкаНаОборудование), "-", "");
КонецФункции

Функция ИмяНастроек()
	Возврат "KSOFT_ЭмуляторККТ_54_ФЗ";
КонецФункции
