...
Функция СтруктураЧекаПоXML(ПараметрыЧека1С) Экспорт
	...
	Для Каждого ДанныеСтрокиЧека Из ПараметрыЧека1С.Positions Цикл
		...
		ИначеЕсли ДанныеСтрокиЧека.Свойство("Barcode") Тогда 
			...
			Попытка
				РезультатФормирования = ГенерацияШтрихкодаВызовСервера.ИзображениеШтрихкода(ПараметрыШтрихкода);
			Исключение
			...
			ПараметрыШтрихкодаСтроки.Картинка = РезультатФормирования.Картинка;
