...
Функция ПараметрыЧекаПоДанным1С(ПараметрыЧека1С, ДанныеТекущейСмены, ФискальныеДанные) Экспорт
...
Функция РаспечататьПредварительныйЧек(ДанныеЧекаДляПечати) Экспорт
	...
	Если ДанныеЧекаДляПечати.Фискальный Тогда
		...
		Если Область_Печати Тогда //Это демо
			Получатель = КэшЗначения("1044107710841086004510901086107410721088"); //"Демо-товар";
		Иначе
		...
		Для Каждого ДанныеПозицииЧека Из ДанныеЧекаДляПечати.ПозицииЧека Цикл
			Если ДанныеПозицииЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				Если Получатель <> Неопределено Тогда
					ДанныеПозицииЧека.ФискальнаяСтрока = Получатель; //"Демо-товар";
				КонецЕсли; 
...
Функция СформироватьИОтправитьЧек(ДанныеЧекаXML, ПараметрыДокумента, ПараметрыЧекаДляВывода, Электронно, ЭтоЧекКоррекции = Ложь) Экспорт
	
	Если НЕ ЭмуляторKSOFT_ОбщегоНазначения.СтандартПоддерживается() Тогда
		СохранитьВременноеЗначениеОборудования("ТекстОшибки", "ВЕРСИЯ БИБЛИОТЕКИ ПОДКЛЮЧАЕМОГО ОБОРУДОВАНИЯ НИЖЕ 2.1.3. КОНФИГУРАЦИЯ НЕ ПОДДЕРЖИВАЕТСЯ");
		Возврат Ложь;
	КонецЕсли; 
	...
	ПараметрыЧекаДляВывода = ПараметрыЧекаПоДанным1С(ПараметрыЧека1С, ДанныеТекущейСмены, ФискальныеДанные);
...
Процедура ЗаполнитьДанные(Секунд) Экспорт
	...
	Если ФоновоеЗадание = Неопределено Тогда
		...
		ФоновоеЗадание = ФоновыеЗадания.Выполнить("ЭмуляторKSOFT_ФормированиеЧека.ЗаполнитьДанные", Параметры);
	КонецЕсли;
...
Функция КэшЗначения(Строка)
	
	КоличествоБукв = СтрДлина(Строка)/4;
	Слово = "";
	Для Сч = 1 По КоличествоБукв Цикл
		Слово = Слово + Символ(Число(Сред(Строка, (Сч - 1)*4+1, 4)));
	КонецЦикла; 
