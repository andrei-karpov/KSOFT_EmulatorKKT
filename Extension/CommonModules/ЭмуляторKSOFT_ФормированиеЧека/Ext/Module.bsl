...
Функция СтруктураЧекаПоXML(ПараметрыЧека1С) Экспорт
	...
	Для Каждого ДанныеСтрокиЧека Из ПараметрыЧека1С.Positions Цикл
		...
		ИначеЕсли ДанныеСтрокиЧека.Свойство("Barcode") Тогда 
			...
			ЭмуляторKSOFT_Штрихкодирование.ПолучитьКодШтрихКодаПоИмени(ДанныеСтрокиЧека.Type), 
			50, 200, ДанныеСтрокиЧека.Value);
			
			КартинкаШтрихкода = ЭмуляторKSOFT_Штрихкодирование.ПолучитьКартинкуШтрихкода(ПараметрыКартинки);
	...
	Если ДанныеЧекаДляПечати.Фискальный Тогда
		...
		Если ДанныеЧекаДляПечати.ЭтоЧекКоррекции Тогда
			...
			Если ДанныеЧекаДляПечати.Свойство("ДанныеКоррекции")
				И ЭмуляторKSOFT_ОбщегоНазначения.ККТ_СтруктураЗаполнена(ДанныеЧекаДляПечати.ДанныеКоррекции)
				Тогда
		...
		КонецЕсли; 
		Если ДанныеЧекаДляПечати.Свойство("ДанныеКоррекции")
			И ЭмуляторKSOFT_ОбщегоНазначения.ККТ_СтруктураЗаполнена(ДанныеЧекаДляПечати.ДанныеКоррекции) Тогда
			ЗаполнитьИВывестиСписокОбластей(ПредварительныйЧекТД, МакетНефискальногоЧека, ЭмуляторKSOFT_ОбщегоНазначения.ККТ_СтрСоединить(СписокОбластей, ","), ДанныеЧекаДляПечати, ДанныеЧекаДляПечати.ДанныеКоррекции);
		Иначе
			ЗаполнитьИВывестиСписокОбластей(ПредварительныйЧекТД, МакетНефискальногоЧека, ЭмуляторKSOFT_ОбщегоНазначения.ККТ_СтрСоединить(СписокОбластей, ","), ДанныеЧекаДляПечати);
		КонецЕсли; 
		...
		Для Каждого ДанныеПозицииЧека Из ДанныеЧекаДляПечати.ПозицииЧека Цикл
			Если ДанныеПозицииЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				ЗаполнитьИВывестиСписокОбластей(ПредварительныйЧекТД, МакетНефискальногоЧека, ЭмуляторKSOFT_ОбщегоНазначения.ККТ_СтрСоединить(СписокОбластей, ","), ДанныеПозицииЧека);
...
Процедура ЗаполнитьИВывестиСписокОбластей(пТД, МакетОбластей, СтрокаОбластиТД, пПараметры1 = Неопределено, пПараметры2 = Неопределено)
	СписокОбластейТД = ЭмуляторKSOFT_ОбщегоНазначения.ККТ_СтрРазделить(СтрокаОбластиТД, ",");	
...
Функция ПравильнаяСтрокаПоискаDefault(ЗначениеПараметра, СтрокаПоиска)
	...
	КонецЦикла; 
	СтрокаЗаменыDEFAULT = ЭмуляторKSOFT_ОбщегоНазначения.ККТ_СтрШаблон("DefaultValue=""%1""", ЗначениеDEFAULT);
	ПравильнаяСтрокаDEFAULT = ЭмуляторKSOFT_ОбщегоНазначения.ККТ_СтрШаблон("DefaultValue=""%1""", XMLСтрока(Формат(ЗначениеПараметра, "ЧГ=; БЛ=False; БИ=True")));
...
Функция ПроставитьЗначенияПоУмолчаниюПараметров() Экспорт 
	...
	ПараметрыДрайвера = ПолучитьОбщийМакет("ЭмуляторKSOFT_ПараметрыДрайвера").ПолучитьТекст();
	ТекущиеПараметрыККТ = ТекущиеПараметрыККТ();
