//Перем ТекстОшибки;
//Перем мПараметрыДрайвера;
//Перем ResultCode Экспорт;
//Перем ResultDescription Экспорт;
//Перем мИмяОперации;
//Перем мТипОбработки;
//Перем мОбработкаСДопФормами;
...
Функция ПолучитьПараметрыККТ(ИДУстройства, ПараметрыККТ) Экспорт  //
	...
	ПараметрыККТ = KSOFT_ФормированиеЧека.ПолучитьXMLОтветНаОснованииСтруктуры("Parameters", KSOFT_ФормированиеЧека.ТекущиеПараметрыККТ());
...
Функция ОткрытьСмену(ИДУстройства, ВходныеПараметры, ВыходныеПараметры) Экспорт
	...
	ВыходныеПараметры = KSOFT_ФормированиеЧека.ПолучитьXMLОтветНаОснованииСтруктуры("OutputParameters.Parameters", ДанныеСмены);
...
Функция СформироватьИОтправитьЧек(ДанныеЧека, ВыходныеПараметрыДокумента, Электронно, ЭтоЧекКоррекции = Ложь)
	...
	ПараметрыЧека1С = KSOFT_ФормированиеЧека.ПараметрыЧека1С(ЭтоЧекКоррекции, ДанныеЧека);	
	Если ПараметрыЧека1С = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли; 
	
	ПараметрыЧека = KSOFT_ФормированиеЧека.СтруктураЧекаПоXML(ПараметрыЧека1С);
	...
	ПараметрыКартинки = Новый Структура("ТипКода, Ширина, Высота, Штрихкод", 16, 300, 300, ТекстQRКода);
	КартинкаQR = KSOFT_Штрихкодирование.ПолучитьКартинкуШтрихкода(ПараметрыКартинки);
	Если КартинкаQR <> Неопределено Тогда
		ПараметрыЧека.КартинкаQR = КартинкаQR;	
	КонецЕсли; 
	...
	ПараметрыДокумента.Вставить("AddressSiteInspections", 	KSOFT_ФормированиеЧека.ТекущиеПараметрыККТ().FNSURL);
	...
	ВыходныеПараметрыДокумента = KSOFT_ФормированиеЧека.ПолучитьXMLОтветНаОснованииСтруктуры("DocumentOutputParameters.Parameters", ПараметрыДокумента);
...
Функция НапечататьТекстовыйДокумент(ИДУстройства, ДанныеДокумента) Экспорт
	...
	ПараметрыЧека = KSOFT_ФормированиеЧека.ДанныеТекстовогоДокумента(ДанныеДокумента);
...
Функция Подключить(пИДУстройства) Экспорт	
	...
	KSOFT_ФормированиеЧека.Подключить();
...
Функция ПолучитьОшибку(пИнформацияОбОшибке) Экспорт
	ПолучитьВременноеЗначениеОборудования("ТекстОшибки");
КонецФункции
...
Функция ПолучитьОписание(ОписаниеДрайвера) Экспорт
	...
	ОписаниеДрайвера = KSOFT_ФормированиеЧека.ПолучитьXMLОтветНаОснованииСтруктуры("DriverDescription", ПараметрыОписанияДрайвера);		
	ТекстОшибки = "";
	Возврат Истина;
КонецФункции
...
Функция ПолучитьПараметры(ПараметрыДрайвера) Экспорт
	...
	ПараметрыДрайвера = KSOFT_ФормированиеЧека.ПроставитьЗначенияПоУмолчаниюПараметров();
...
Функция УстановитьПараметр(ИмяПараметра, ЗначениеПараметра) Экспорт
	KSOFT_ФормированиеЧека.УстановитьЗначениеПараметра(ИмяПараметра, ЗначениеПараметра); 
	Возврат Истина;
КонецФункции
...
Функция ОперацияФН(ИДУстройства, ТипОперации, ПараметрыФискализацииXML) Экспорт
	...
	KSOFT_ФормированиеЧека.ОперацияФН(ПараметрыФискализацииXML);
...
Функция СтруктураКассира(пПараметрыОперации)
	ДанныеКассира = KSOFT_ФормированиеЧека.ПолучитьСтруктуруКассира(пПараметрыОперации);
...
Процедура ВывестиДополнительныйЧек(ТипОперации, ДанныеКассира = Неопределено, Сумма = 0)
	...
	ПараметрыДопЧека.Вставить("ДокументЧекККМ", KSOFT_ФормированиеЧека.ТекстовыйДокументЧек(ДанныеКассира, ТипОперации, Сумма));
...
Процедура ВывестиЧекНаФорму(ДанныеЧекаДляПечати, XMLПредварительныйЧек)
	...
	ПредварительныйЧек = KSOFT_ФормированиеЧека.РаспечататьПредварительныйЧек(ДанныеЧекаДляПечати);
	...
	Если ПараметрыДрайвера.Свойство("TypeOpenWindow") И ПараметрыДрайвера.TypeOpenWindow = 1 Тогда
		ОткрытьФорму("ОбщаяФорма.ЭмуляторKSOFT_ПредварительныйЧек", ПараметрыДопЧека,,,,,,РежимОткрытияОкнаФормы.Независимый);
	Иначе
		ОткрытьФорму("ОбщаяФорма.ЭмуляторKSOFT_ПредварительныйЧек", ПараметрыДопЧека,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли; 								
...
Функция ФорматЧислаЗаданногоРазмера(ЧислоДляФорматирования, ЗаданныйРазмер = 0)
	...
	Если ЗначениеЗаполнено(ЗаданныйРазмер) И СтрДлина(ОтформатированноеЧисло) < ЗаданныйРазмер Тогда 	
		Возврат Формат(ЧислоДляФорматирования, KSOFT_ОбщегоНазначенияКлиент.ККТ_СтрШаблон("ЧЦ=%1; ЧВН=; ЧГ=", ФорматЧислаЗаданногоРазмера(ЗаданныйРазмер)));
	КонецЕсли; 
