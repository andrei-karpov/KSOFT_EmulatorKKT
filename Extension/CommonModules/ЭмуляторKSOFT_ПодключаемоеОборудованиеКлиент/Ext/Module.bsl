...
Функция ПолучитьПараметрыККТ(ПараметрыККТ, ТекстОшибки) Экспорт  //
	
	ПараметрыККТ = ЭмуляторKSOFT_ФормированиеЧека.ПолучитьXMLОтветНаОснованииСтруктуры("TableParametersKKT", ЭмуляторKSOFT_ФормированиеЧека.ТекущиеПараметрыККТ());
...
Функция СформироватьЧек(ИДУстройства, Электронно, ДанныеЧека, ВыходныеПараметрыДокумента, ПараметрыДрайвера, ТекстОшибки) Экспорт 
	...
	Если РезультатОперации Тогда
		...
		ВывестиЧекНаФорму(ПараметрыЧекаДляВывода, ДанныеЧека);	
...
Функция НапечататьТекстовыйДокумент(ИДУстройства, ДанныеДокумента, ТекстОшибки) Экспорт
...
Функция ПолучитьПараметры(ПараметрыДрайвера, ТекстОшибки) Экспорт
	
	ПараметрыДрайвера = ЭмуляторKSOFT_ФормированиеЧека.ПроставитьЗначенияПоУмолчаниюПараметров(ПараметрыДрайвераXML());
...
Функция ВыполнитьДополнительноеДействие(пИмяДействия, ПараметрыДрайвера, ТекстОшибки) Экспорт
	...
	ИначеЕсли Врег(пИмяДействия) = "OPENSHIFT" Тогда
		ОткрытьСмену(0, Неопределено, Неопределено, ПараметрыДрайвера, ТекстОшибки);
		Возврат Истина;
	ИначеЕсли Врег(пИмяДействия) = "CLOSESHIFT" Тогда 	
		ЗакрытьСмену(0, Неопределено, Неопределено, ПараметрыДрайвера, ТекстОшибки);
		Возврат Истина;
	КонецЕсли; 
...
Процедура ВывестиЧекНаФорму(ДанныеЧекаДляПечати, Знач XMLПредварительныйЧек)
...
Функция ОбъектДрайвераЭмулятор(ДанныеОборудования) Экспорт
	...
	ОбъектДрайвера.ИдентификаторОборудования = СериализаторXDTO.XMLСтрока(ДанныеОборудования.Ссылка);
