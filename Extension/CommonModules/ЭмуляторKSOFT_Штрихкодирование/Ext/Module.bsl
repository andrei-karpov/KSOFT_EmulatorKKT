...
Функция Штрихкод_EAN8(ТекстШтрихкода)
	...
	Возврат ЭмуляторKSOFT_ОбщегоНазначения.ККТ_СтрШаблон("111%111111%2%3111", ЛеваяЧасть, ПраваяЧасть, КодыEAN8[КонтрольныйСимвол]);
...
Функция КэшБайтов() 
	КэшБайтов = ЭмуляторKSOFT_ОбщегоНазначения.ПолучитьВременноеЗначениеОборудования("КэшБайтов");
	...
	КонецЦикла; 
	ЭмуляторKSOFT_ОбщегоНазначения.СохранитьВременноеЗначениеОборудования("КэшБайтов", КэшБайтов);
	Возврат КэшБайтов;
КонецФункции
...
Функция ПолучитьКартинкуШтрихкода(ПараметрыШтрихкода) Экспорт	
	Если ПараметрыШтрихкода.ТипКода <> 16 Тогда
		 Возврат ЭмуляторKSOFT_Штрихкодирование.ПолучитьКартинкуЛинейногоШК(ПараметрыШтрихкода.Штрихкод, ПараметрыШтрихкода.ТипКода, ПараметрыШтрихкода.Ширина, ПараметрыШтрихкода.Высота);
	Иначе	 
